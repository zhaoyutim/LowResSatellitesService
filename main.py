...
if __name__ == '__main__':
    ...
    for location in locations:
        ...
        #     laads_client.download_files_to_local_based_on_filelist(date, products_id=products_id, day_night_modes=['N'], collection_id=collection_id, data_path='data/VNPL1')
        for i in range(3):
            date = (datetime.datetime.strptime(start_date, '%Y-%m-%d')+datetime.timedelta(i)).strftime('%Y-%m-%d')
            pipeline = Pipeline()
