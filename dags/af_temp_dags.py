...
sources = ["VIIRS","MODIS"]
urls = [["https://firms.modaps.eosdis.nasa.gov/data/active_fire/noaa-20-viirs-c2/shapes/zips/J1_VIIRS_C2_Global_7d.zip","https://firms.modaps.eosdis.nasa.gov/data/active_fire/noaa-21-viirs-c2/shapes/zips/J2_VIIRS_C2_Global_7d.zip"],
...
def download_and_upload(urls, save_folder, asset_id, gspath):
    ...
    for url in urls:
        ...
        new_file_path = os.path.join(save_folder,new_file_name+".shp")
        paths.append(new_file_path)

    
    print("Reading files....")
    new_file = geopandas.read_file(paths[0],engine='pyogrio', use_arrow=True)
    print(new_file.columns)
    new_file['ACQ_DATE'] = pandas.to_datetime(new_file['ACQ_DATE']).astype(np.int64) // 10**6
    old_file = geopandas.read_file(paths[1],engine='pyogrio', use_arrow=True)
    print("Merging files...")
...
for i in range(len(sources)):
    dag = DAG(
        f'{sources[i]}_AF_process_and_upload_temp',
    ...
    with dag:
        download_task = PythonOperator(
            ...
            op_kwargs={
                ...
                'asset_id':f"projects/ee-eo4wildfire/assets/{sources[i]}_AF_2024_7D",
