...
if __name__ == '__main__':
    ...
    # file_list = glob(file_dir+'\\203\\*\\*.tif')
    # file_list.sort()
    ...
    for file in file_list:
        ...
        if int(time)<=1200:
            cmd = 'earthengine upload image --time_start '+time_start+' --asset_id=projects/grand-drive-285514/assets/viirs_night/'+file.split('\\')[-1][:-4]+' --pyramiding_policy=sample gs://ai4wildfire/'+'VNPIMGTIF/2021'+date+'/'+file_name
            subprocess.call(cmd.split())
        else:
            cmd = 'earthengine upload image --time_start '+time_start+' --asset_id=projects/grand-drive-285514/assets/viirs_day/'+file.split('\\')[-1][:-4]+' --pyramiding_policy=sample gs://ai4wildfire/'+'VNPIMGTIF/2021'+date+'/'+file_name
            subprocess.call(cmd.split())