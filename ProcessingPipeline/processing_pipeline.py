...
class Pipeline:
    ...
    def cloud_optimization(self, date, file, day_night_modes=['D', 'N']):
        ...
        #     os.mkdir('data/cogtif/' + date)
            ...
            os.system('rio cogeo create ' + file + ' data/VNPIMGTIF/' + date + '/' +day_night+'/'+ file.split('/')[-2] + '/' + file_name)
            print('Cloud Optimization Finish')
        # else:
        #     print('Cloud Optimized Geotiff already exists')

    def upload_to_gcloud(self, date, file, day_night_modes=['D', 'N']):
        ...
        for day_night in day_night_modes:
            ...
            upload_cmd = 'gsutil cp ' + file + ' gs://ai4wildfire/' + 'VNPIMGTIF/'+year + date + '/'+day_night+'/' + file_name
            print(upload_cmd)
    ...
    def upload_to_gee(self, date, file, day_night_modes=['D', 'N']):
        ...
        for day_night in day_night_modes:
            if day_night=='N':
                cmd = 'earthengine upload image --time_start ' + time_start + ' --asset_id=projects/ee-zhaoyutim/assets/proj3_test_night/' + \
                      file.split('/')[-1][
                      :-4] + ' --pyramiding_policy=sample gs://ai4wildfire/' + 'VNPIMGTIF/'+year + date + '/' +day_night+'/'+ file_name
                subprocess.call(cmd.split())
            else:
