...
class Pipeline:
    ...
    def upload_to_gee(self, date, file):
        ...
        if int(time)>800 and int(time)<1800:
            cmd = 'earthengine upload image --time_start ' + time_start + ' --asset_id=projects/ee-zhaoyutim/assets/sydney_fire_night/' + \
                  file.split('/')[-1][
                  :-4] + ' --pyramiding_policy=sample gs://ai4wildfire/' + 'VNPIMGTIF/'+year + date + '/' + file_name
            subprocess.call(cmd.split())
        else:
            cmd = 'earthengine upload image --time_start ' + time_start + ' --asset_id=projects/ee-zhaoyutim/assets/sydney_fire_day/' + \
                  file.split('/')[-1][
                  :-4] + ' --pyramiding_policy=sample gs://ai4wildfire/' + 'VNPIMGTIF/'+year + date + '/' + file_name
            subprocess.call(cmd.split())
