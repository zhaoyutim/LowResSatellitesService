...
if __name__ == '__main__':
    ...
    for location in locations:
        ...
        #     laads_client.query_filelist_with_date_range_and_area_of_interest(date, collection_id=collection_id, area_of_interest=area_of_interest)
        #     laads_client.download_files_to_local_based_on_filelist(date, collection_id=collection_id)
        for i in range(1):
            date = (datetime.datetime.strptime(start_date, '%Y-%m-%d')+datetime.timedelta(i)).strftime('%Y-%m-%d')
            pipeline = Pipeline()
            pipeline.processing(date, roi, utmzone)